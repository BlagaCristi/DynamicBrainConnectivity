...
class GraphWavenetModel(nn.Module):
    def __init__(self, num_nodes, device, dropout = 0.3, supports = None, gcn_bool = True, addaptadj = True,
        ...
        if gcn_bool and addaptadj:
            if aptinit is None:
                if supports is None:
                    self.supports = []
                self.nodevec1 = nn.Parameter(torch.randn(num_nodes, 10).to(device).float(), requires_grad = True).to(device)
                self.nodevec2 = nn.Parameter(torch.randn(10, num_nodes).to(device).float(), requires_grad = True).to(device)
                self.supports_len += 1
            else:
                if supports is None:
                    self.supports = []
    ...
    def forward(self, input):
        ...
        for i in range(self.blocks * self.layers):
            ...
            if self.gcn_bool and self.supports is not None:
                if self.addaptadj:
                    x = x + self.gconv[i](x, new_supports)
                else:
                    x = x + self.gconv[i](x, self.supports)
            else:
    ...
    def get_node_embeddings(self):
        return self.nodevec1, self.nodevec2, F.softmax(F.relu(torch.mm(self.nodevec1, self.nodevec2)), dim = 1)
